...
function NewsDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      setStatus('connected');
      socket.emit('news:subscribe', { minImportance: 6 });
    });
...
function PortfolioTracker() {
  ...
  const addSymbol = () => {
    if (newSymbol && !portfolio.includes(newSymbol.toUpperCase())) {
      ...
      if (socket) {
        socket.emit('news:subscribe', { symbol, minImportance: 6 });
      }
...
Create `.env` file:
```env
REACT_APP_BYUL_API_KEY=byul_v2_your_api_key
```
...
function useByulNewsWithErrors() {
  ...
  useEffect(() => {
    ...
    socket.on('news:error', (err) => {
      setError(`Authentication failed: ${err.message}`);
      setStatus('error');
    });

    socket.on('news:error', (err) => {
      setError(`News error: ${err.message}`);
    });
